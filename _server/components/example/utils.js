    ...
    async onPress() {
      ...
      if (navigator.msSaveOrOpenBlob) {
        ...
        reader.onloadend = () => {
          ...
          cleanup = downloadDocument(dataUrl, extension);
        };

        reader.readAsDataURL(blob);
      } else {
        const objectUrl = window.URL.createObjectURL(blob);
        const cleanupAct1 = downloadDocument(objectUrl, extension);
      ...
      // Cleaning up immediately leads to WebkitBlobResource error 1, so we need
